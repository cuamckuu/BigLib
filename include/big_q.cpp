...
BigFra::BigFra(std::string str):numerator("0"), denominator("1"){
...
BigFra support_fac_p_q(std::vector<BigFra> elem) {
    if(elem.size() == 1){
        return elem[0];
    } else {
        BigNat res1(GCD_NN_N(elem[0].denominator, elem[1].denominator));
        BigNat res2(LCM_NN_N(elem[0].numerator, elem[1].numerator));
        int i = 2;

        std::cout << "res 1 and 2: " << res1 << "    " << res2 << std::endl;

        while (i < elem.size()){
            res1 = (GCD_NN_N(res1, elem[i].denominator));
            res2 = (LCM_NN_N(res2, elem[i].numerator));
            i++;
        }
        return BigFra(BigInt(res2, false), BigInt(res1, false));
    }
}
//==OPERATORS===
